  ...
  if (((t.__M = t.__M || {}), !t.__M.require)) {
    ...
    const l = function (t, n) {
      if (!(t in u)) {
        ...
        if (n) {
          const o = setTimeout(n, e.timeout);
          i.onerror = function () {
            clearTimeout(o), n();
          };
          const a = function () {
    ...
    (n = function (t, e) {
      typeof e !== 'function' && (e = arguments[2]),
        (t = t.replace(/\.js$/i, '')),
        (o[t] = e);
      const n = i[t];
      if (n) {
        for (let r = 0, a = n.length; a > r; r++) n[r]();
        delete i[t];
      }
    }),
      (e = function (t) {
...
// 定义douyin_falcon:node_modules/byted-acrawler/dist/runtime
global.__M.define(
...
// 定义_bytedAcrawler
const _bytedAcrawler = global.__M.require(
  'douyin_falcon:node_modules/byted-acrawler/dist/runtime'
);
...
const getSignature = async (uid) => {
  ...
  global.tac = tac;
  let signature = _bytedAcrawler.sign(uid);
  return { signature, dytk };
}
 ...
 * @param {*} uid
 */
const getTac = async (uid) => {
  const url = `https://www.iesdouyin.com/share/user/${uid}`;
  const options = {
    url,
  };
    ...
    try {
      request(options, (err, res) => {
        ...
        if (tacReg.test(content)) {
          let tacContent = RegExp.$1;
          tacContent = tacContent
            .replace(/`/g, '``')
            .replace(/^'/, '')
            .replace(/'$/, '');
          tacContent = decode(tacContent);
          signInfo.tac = tacContent;
        }
        if (dytkReg.test(content)) {
          const dytkContent = RegExp.$1;
          signInfo.dytk = dytkContent;
        }
        resolve(signInfo);
      });
    } catch (error) {
      resolve(signInfo);
      console.log('tac-error', error);
    }
  });
};
